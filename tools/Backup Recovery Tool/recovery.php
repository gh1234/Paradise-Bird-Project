 ...
 */
/**
 * Stellt PDBP Backups (ZIP Dateien) wieder her
 */
echo '<p>This is the Paradise Bird Project backup recovery tool, lets see what we can do for you! By the way, we are sorry, there are no locals for other languages, but this tool should explain itself without problems.</p>';
echo '<p>Use ?overwrite=true, to overwrite existing files.</p>';
echo '<p>Database Recovery is not supported yet. Coming soon!</p>';
echo '<p>So... lets get ready to rescue your files!</p> <hr>';
if(!file_exists('./backup.zip'))
die("You need to upload <b>backup.zip</b>!");
$zip = new PclZip('./backup.zip');
$file_list = $zip->listContent();
if(isset($_GET['overwrite']) && $_GET['overwrite'] == true){
	foreach($file_list as $file){
		if(file_exists($file['filename']) && !is_dir($file['filename']) && is_writeable($file['filename']))
		unlink($file['filename']);
	}
} else {
	echo '<p>If you want to overwrite existing files, load the page with "?overwrite=true", or click <a href="?overwrite=true">here</a>.</p>';
}
if($zip->extract() == 0)
die('Error while unpacking.');
else
die('Backup installed');
?>